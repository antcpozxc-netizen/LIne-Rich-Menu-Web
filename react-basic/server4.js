...
// ---------- 5) ส่ง index แบบ no-cache (ไม่ดัดแปลง, ไม่ลบ/แทรก <script>) ----------
function sendIndexNoCache(req, res) {
  ...
  // 2) เคลียร์ cache/storage อัตโนมัติเมื่อเปิดผ่าน LINE (WKWebView)
  const ua = String(req.headers['user-agent'] || '');
  if (/\bLine\/\d/i.test(ua)) {
    res.set('Clear-Site-Data', '"cache", "storage"');
  }

  // 3) อ่านแล้วส่งไฟล์ index.html ตรงๆ (ไม่ดัดแปลง)
  try {
  ...
  try {
    ...
    if (userAreasMsg[3]) {
      ...
      userAreasMsg[3] = {
        ...
        action: { type:'message', text:'ลงทะเบียนเข้าใช้งาน' }
      };
    }
    ...
    // 3) เคลียร์ default เก่า แล้วตั้ง default OA เป็นเมนู USER + ถ้าเป็น admin/supervisor ให้ลิงก์เมนู admin ให้ผู้ที่สั่ง enable
    ...
    try {
      ...
      if (me) {
        ...
        let role = 'user';
        try {
          const gu = await callAppsScriptForTenant(tenant.ref, 'get_user', { user_id: me }, { sheetFrom: 'attendance' })
                        .catch(() => ({}));
...
async function handleLineEvent(ev, tenantRef, accessToken) {
  ...
  if (ev.type === 'message' && ev.message?.type === 'text') {
    ...
    if (/^(ลงทะเบียนเข้าใช้งาน|ลงทะเบียน)$/i.test(text)) {
...
async function ensureRichMenu(tenantRef, kind, imageUrl, areasPx) {
  ...
  if (imageUrl) {
    ...
    try {
      const d = await getRichMenuDetailByToken(accessToken, richMenuId); // <-- แก้เป็น accessToken
...
function buildAdminAreas() {
  ...
  const G_X = 40, G_Y = 60;
  const RIGHT_W = CANVAS_W - LEFT_BANNER_W;
  const TILE_W  = Math.floor((RIGHT_W - (3 * G_X)) / 2);
  const TILE_H  = Math.floor((CANVAS_H - (3 * G_Y)) / 2);
  const X1 = LEFT_BANNER_W + G_X;
  const X2 = X1 + TILE_W + G_X;
  const Y1 = G_Y;
  const Y2 = Y1 + TILE_H + G_Y;
  ...
  return [
    ...
    { bounds:{ x:X2, y:Y2, width:TILE_W, height:TILE_H }, action:{ type:'message', text:'ตั้งค่า' } },
  ];
}

function buildUserAreas() {
  const CANVAS_W = 2500, CANVAS_H = 1686;
  const LEFT_BANNER_W = 900;
  const G_X = 40, G_Y = 60;
  const RIGHT_W = CANVAS_W - LEFT_BANNER_W;
  ...
  return [
    ...
    { bounds:{ x:X2, y:Y2, width:TILE_W, height:TILE_H }, action:{ type:'message', text:'ลงทะเบียนเข้าใช้งาน' } },
  ];
}
...
async function applyAttendanceRichMenus(tenant, cfg) {
  ...
  // ใช้ preset ที่ประกาศไว้แล้ว (message-only)
  const adminAreas = buildAdminAreas();
  const userAreas  = buildUserAreas();

  const adminRM = await ensureRichMenu(
    tenantRef, 'ATTEND_MAIN_ADMIN',
    cfg.adminMenuImageUrl || ATTEND_ADMIN_IMG,
    adminAreas
  );
  const userRM  = await ensureRichMenu(
    tenantRef, 'ATTEND_MAIN_USER',
    cfg.userMenuImageUrl  || ATTEND_USER_IMG,
    userAreas
  );
