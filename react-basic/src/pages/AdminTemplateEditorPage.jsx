...
function TemplateModal({ open, onClose, value, onApply }) {
  ...
  const render = (t) => (
    <Grid key={t.id} item xs={12} sm={6} md={4}>
      <Paper variant="outlined" onClick={() => setSelected(t.id)}
  ...
  return (
    <Stack spacing={1}>
      <TextField select size="small" label="Type" value={action?.type || 'Link'}
        onChange={(e)=>onChange({ ...(action||{}), type:e.target.value })} SelectProps={{ native:true }}>
      ...
      {(action?.type==='Text') && (
        <TextField size="small" label="Message" value={action.text||''}
          onChange={(e)=>onChange({ ...action, text:(e.target.value||'').slice(0,300) })} fullWidth/>
      )}
    </Stack>
  );
}
          ...
          <Paper variant="outlined" sx={{ p:2 }}>
            ...
            <Stack spacing={1.5}>
              ...
              <TextField label="Chat bar label" value={form.chatBarText}
                         onChange={(e)=>setForm(f=>({ ...f, chatBarText:(e.target.value||'').slice(0,14) }))} fullWidth/>
              ...
              <TextField label="Tags (comma)" value={form.tags}
                         onChange={(e)=>setForm(f=>({ ...f, tags:e.target.value }))} fullWidth/>
              <TextField label="Note" value={form.note}
                         onChange={(e)=>setForm(f=>({ ...f, note:e.target.value }))} fullWidth multiline/>
